...
python backend/tests/e2e/api_tests.py        # e2e + snapshot 268 เคส (ต้องรัน API ก่อน)
...
- **รายการคะแนนจัดการที่ระดับภาคเรียนเท่านั้น** (`/teacher/terms/:termId/items` · `TermItemEndpoints.cs`): เพิ่ม/แก้ชื่อ/แก้คะแนนเต็ม/ซ่อน-แสดง/ลบ/ปัดคะแนน ทำทีเดียวทุกห้อง · ระดับห้องเหลือแค่ดูรายการ (GET) กับลบรายการเดี่ยว · แท็บของห้องเหลือ นักเรียน / ตารางคะแนน
...
- **1 ภาคเรียน = 1 ชั้น** (ตกลงกับผู้ใช้ 2026-09-20): ระบบมีแค่ ภาคเรียน → ห้อง ไม่มีระดับ "ชั้น" ของตัวเอง · ไฟล์ครู 1 ไฟล์ = 1 ชั้น (ม.1 ทั้ง 8 ห้อง) พอขึ้น ม.2 ให้สร้างภาคเรียนใหม่แล้วนำเข้าอีกไฟล์ · คำสั่งระดับภาคเรียน (ซ่อน/แสดง, ปัดคะแนน, ดูคะแนนด่วน) จึงมีผลเท่ากับทั้งชั้นพอดี · **ถ้าวันหนึ่งมีหลายชั้นปนในภาคเรียนเดียว ต้องเพิ่มตัวกรองห้องในหน้า `/teacher/terms/:termId/items` ก่อน** ไม่งั้นครูจะเผลอสั่งข้ามชั้น
- **จัดการรายการคะแนนระดับภาคเรียน** (`/teacher/terms/:termId/items` · `Endpoints/TermItemEndpoints.cs`): รวมรายการที่ชื่อตรงกันของทุกห้องเป็นแถวเดียว · สวิตช์ซ่อน/แสดงมีผลทุกห้องพร้อมกัน · ค่าที่ไม่ตรงกันทุกห้องส่ง `null` (เว็บแสดงว่าคละกัน) · ครูนำเข้าไฟล์ทีเดียว 8 ห้อง การไล่กดทีละห้องคืองานซ้ำ 8 รอบ
...
- Smart App Control บล็อก `Api.dll` แบบ Debug บ่อยขึ้นเรื่อย ๆ (`0x800711C7`) จน `dotnet test` รัน 0 เทสต์ · **ทางที่ได้ผลชัวร์สุดคือใช้ Release**: `dotnet build -c Release -p:Deterministic=false` แล้ว `dotnet test -c Release` และรัน API จาก `bin/Release/net10.0/Api.dll`
